...
from .transformers import LlamaForCausalLM, LlamaConfig, WeightTensor
...
class LLaMAMixDAStageOneCLM(LightningModule):
    @staticmethod
    def add_model_specific_args(parent_parser):
        parser = ArgumentParser(parents=[parent_parser], add_help=False)
        parser.add_argument(
            "--dirpath", type=str, default="llama_translated_7_11"
        )
        parser.add_argument(
            "--knowledge_data_path",
            type=str,
            default="datasets/biomed/translated_biomed.json",
        )  
        ...
        parser.add_argument("--model_name", type=str, default="/data/weiguang/Llama2-Chinese-7b-Chat")
        parser.add_argument(
            "--model_checkpoint",
            type=str,
            default="",
        ...
        parser.add_argument("--adapter_module", type=str, default='model.layers.{}.mlp.kas')
        parser.add_argument("--mlp_module", type=str, default='model.layers.{}.mlp')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tokenizer = LlamaTokenizerFast.from_pretrained(self.hparams.model_name) # token='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'
        ## newly added
        ...
        model_config =  LlamaConfig.from_pretrained(
            ...
            #token='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',
        )
        model_config.max_position_embeddings = 2048
        self.tokenizer.model_max_length = model_config.max_position_embeddings ## added as we initialized the tokenizer from local folder
        
        self.model = LlamaForCausalLM.from_pretrained(self.hparams.model_name, config=model_config) # token='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').to(self.device)
        ...
        for layer in self.hparams.layers:
            ...
            layer_list.append('model.model.layers.{}.mlp.gating'.format(layer))
    ...
    def train_dataloader(self, shuffle=False):
        ...
        if not hasattr(self, 'sample_dataset'):
            ### load wiki data in Chinese from local files
            file_pattern = '/data/weiguang/20231101.zh/wikipedia-train-*.arrow' 
            files_list = [x.split('/')[-1] for x in glob.glob(file_pattern)]
            sample_dataset_raw = load_dataset('arrow', data_dir='/data/weiguang/20231101.zh',
                                              data_files={"train": files_list})
            ### load wiki data in English from local files
            # sample_dataset_raw = load_dataset('arrow', data_dir='/data/weiguang/20220301.en/2.0.0/aa542ed919df55cc5d3347f42dd4521d05ca68751f50dbc32bae2a7f1e167559',
            #                                   data_files={"train": 'wikipedia-train.arrow'})
            ### load wiki data in English from huggingface
    ...
    def training_step(self, batch, batch_idx=None):
        ...
        sample_layers = ['model.layers.{}'.format(
        ...
        if not self.hparams.no_old_knowledge:
            ...
            for layer in self.hparams.layers:
                layer_adapter_logits, layer_mlp_logits = \
                    tr['model.layers.{}'.format(layer)].output
