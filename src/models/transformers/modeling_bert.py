...
class BertOutput(BertOutputAdaptersMixin, nn.Module):
    def __init__(self, config, layer):
        ...
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.enabled = layer in config.layers
        self.adapter = MixtureOfDomainAdapter(config) if self.enabled else None
        self.layer = layer
...
class BertLayer(nn.Module):
    def __init__(self, config, layer):
        ...
        self.output = BertOutput(config, layer)
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def save_knowledge_adapter(self, f):
        ...
        for i, layer in enumerate(self.encoder.layer):
            if layer.output.adapter is not None:
                dict[str(i) + '-adapter'] = layer.output.adapter.state_dict()
        torch.save(dict, f)
    
    def load_knowledge_adapter(self, f):
        checkpoint = torch.load(f)
        for i, layer in enumerate(self.encoder.layer):
            if str(i) + '-adapter' in checkpoint:
                layer.output.adapter.load_state_dict(checkpoint[str(i) + '-adapter'])
