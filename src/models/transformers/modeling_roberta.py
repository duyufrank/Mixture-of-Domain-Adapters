...
from transformers.adapters.mixins.bert import BertModelAdaptersMixin, BertOutputAdaptersMixin, BertSelfOutputAdaptersMixin
...
# from adapters.models.bert.mixin_bert import BertModelAdaptersMixin, BertOutputAdaptersMixin, BertSelfOutputAdaptersMixin
...
class RobertaLayer(nn.Module):
    def __init__(self, config, layer):
        ...
        self.output = RobertaOutput(config, layer)
    ...
    def forward(
        ...
        layer_output, original_output = apply_chunking_to_forward(
        ...
        return outputs, original_output

    def feed_forward_chunk(self, attention_output):
        # TODO: update new KA for multi adapter
        adapter_output = None
        if self.ka is not None:
            adapter_output = self.ka(attention_output)
            adapter_output = self.dropout(adapter_output)
            gating_weights = self.gating(attention_output)
            adapter_output = self.LayerNorm(adapter_output + attention_output)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        if type(layer_output) is tuple and len(layer_output) == 2:
            layer_output = layer_output[0]
        if adapter_output is not None:
            final_output = torch.stack([adapter_output, layer_output], dim=3) @ gating_weights.unsqueeze(3)
            final_output = final_output.squeeze()
            # final_output = adapter_output
        else:
            final_output = layer_output
        return final_output, layer_output
...
class RobertaPreTrainedModel(PreTrainedModel):
    ...
    def save_knowledge_adapter(self, f):
        ...
        for i, layer in enumerate(encoder.layer):
            if layer.output.kas is not None:
                for j, adapter in enumerate(layer.output.kas):
    ...
    def forward(
        ...
        if token_type_ids is None:
            if hasattr(self.embeddings, "token_type_ids"):
                ...
                token_type_ids = buffered_token_type_ids_expanded.to(device)
            else:
