...
import math
import warnings
from turtle import forward ## from mixture/modeling_roberta.py
...
## from mixture/modeling_roberta.py
...
from .bert import BertModelAdaptersMixin, BertOutputAdaptersMixin
...
from transformers.utils import (
    add_code_sample_docstrings, ## from mixture/modeling_roberta.py
...
from ..mixda_adapter import * ## from mixture/modeling_roberta.py
...
class LlamaMLP(BertOutputAdaptersMixin, nn.Module):
    def __init__(self, config, layer):
        ...
        if self.enabled:
            for _ in range(config.num_of_kas):
                adapter_list.append(MixtureOfDomainAdapter(config, input_size=config.hidden_size))
            self.kas = nn.ModuleList(adapter_list)
        self.gating = MoeGating(config.hidden_size, 768, len(adapter_list) + 1, 3, 0.2) if self.enabled and not config.disable_moe else None
    ...
    def forward(self, x):
        ...
        if self.enabled and self.kas is not None:
            ...
            for adapter in self.kas:
                all_results.append(adapter(x))
        if self.gating is not None:
            gating_weights = self.gating(x)
            
        if all_results is not None:
            all_results.append(down_proj)
            if self.gating is not None:
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = (
        ...
        self.mlp = LlamaMLP(config, layer)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.ka is not None:
            adapter_output = self.ka(hidden_states)
            adapter_output = self.dropout(adapter_output)
            gating_weights = self.gating(hidden_states)
            adapter_output = self.LayerNorm(adapter_output + hidden_states)
        ## newly added
        ...
        ## newly added as config.output_original = True
        if type(hidden_states) is tuple and len(hidden_states) == 2:
        ...
        if adapter_output is not None:
            final_output = torch.stack([adapter_output, hidden_states], dim=3) @ gating_weights.unsqueeze(3)
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    ## never called, and not sure the name of the corresponding module            
    ...
    ## need the structure of llama model        
    # def enable_adapter(self, enable):
    #     if hasattr(self, "llama"):
    #         for layer in self.llama.layers:
    #             layer.output.enable_adapter(enable)
    #     else:
    #         for layer in self.layers:
    #             layer.output.enable_adapter(enable)
    
    def save_knowledge_adapter(self, f):
        ...
        for i, layer in enumerate(self.model.layers):
            if layer.mlp.kas is not None:
                for j, adapter in enumerate(layer.mlp.kas):
                    dict[str(i) + '-' + str(j) + '-adapter'] = adapter.state_dict()
            if layer.mlp.gating is not None:
                dict[str(i) + '-gating'] = layer.mlp.gating.state_dict()
        torch.save(dict, f)
        ...
        for n_ka, f in enumerate(f_list):
            ...
            for i, layer in enumerate(self.model.layers):
                if str(i) + '-adapter' in checkpoint:
                    layer.mlp.kas[n_ka].load_state_dict(checkpoint[str(i) + '-adapter'])
                ka_temp = 0
                while True:
                    if str(i) + '-' + str(ka_temp) + '-adapter' in checkpoint:
                        layer.mlp.kas[n_ka].load_state_dict(
                ...
                if str(i) + '-gating' in checkpoint and len(f_list) == 1 and layer.mlp.gating is not None:
                    layer.mlp.gating.load_state_dict(checkpoint[str(i) + '-gating'])
        if one_f is not None:
            ...
            for i, layer in enumerate(self.layers):
                ...
                if str(i) + '-gating' in checkpoint:
                    layer.mlp.gating.load_state_dict(
...
class LlamaModel(BertModelAdaptersMixin, LlamaPreTrainedModel):
    """
...
class LlamaForCausalLM(ModelWithHeadsAdaptersMixin, LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        ### newly added: we dont have decoder in LM head
...
class LlamaForSequenceClassification(ModelWithHeadsAdaptersMixin, LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
